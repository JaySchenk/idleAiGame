...
describe('HCUDisplay', () => {
  describe('Basic Rendering', () => {
    it('should render amount with default unit', () => {
      const wrapper = mount(HCUDisplay, {
        props: { amount: 100 }
      })
      
      expect(wrapper.text()).toBe('100.00 HCU')
      expect(wrapper.find('.hcu-display').exists()).toBe(true)
    })
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle maximum safe integer', () => {
      const wrapper = mount(HCUDisplay, {
        props: { amount: Number.MAX_SAFE_INTEGER }
      })
    ...
    it('should handle minimum safe integer', () => {
      const wrapper = mount(HCUDisplay, {
        props: { amount: Number.MIN_SAFE_INTEGER }
      })
