...
export class UnlockSystem {
   ...
   */
  static checkConditions(conditions: UnlockCondition[], gameState: GameState): UnlockResult {
  ...
  private static checkPrestigeCondition(
    ...
    return {
      isUnlocked,
      isVisible: visible,
      failedConditions: isUnlocked
        ? []
        : [`Prestige level (${currentLevel}) ${comparison} ${condition.minPrestigeLevel}`],
    }
  }
  ...
  private static checkTimeCondition(
    ...
    return {
      ...
      failedConditions: isUnlocked
        ? []
        : [
            `Play time (${Math.floor(currentTime / 1000)}s) ${comparison} ${Math.floor(condition.minPlayTime / 1000)}s`,
          ],
    }
  }
  ...
  private static compareValues(actual: number, target: number, comparison: string): boolean {
    switch (comparison) {
      case '>':
